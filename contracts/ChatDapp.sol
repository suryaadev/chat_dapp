...
contract ChatDapp{
    ...
    function addfriend(address _friendAddress, string calldata _name) external{
        ...
        require(checkUserExist(_friendAddress), "friend account does not exist");
        ...
        require(checkAlreadyFriend(msg.sender, _friendAddress) == false, "you guys are already friend");
    ...
    function checkAlreadyFriend(address _userAddress, address _friendAddress) internal view returns(bool){
        // here we see which list is bigger and comparing that whole list with the single address
        if(userList[_userAddress].friendList.length > userList[_friendAddress].friendList.length){
            address tmp = _userAddress;
            _userAddress = _friendAddress;
            _friendAddress = tmp;
        }
        for(uint256 i=0; i< userList[_userAddress].friendList.length; i++){
            if(userList[_userAddress].friendList[i].friend_address == _friendAddress){
                return true;
            }
            else {
                return false;
    ...
    function _getChatCode(address _userAddress, address _friendAddress) internal pure returns(bytes32){ 
    ...
    function sendMessage(address _friendAddress, string memory _msg) external{
        ...
        require(checkAlreadyFriend(msg.sender,_friendAddress), "send request first");
    ...
    function readMessages(address _friendAddress) external view returns(message[] memory){
        bytes32 chatCode = _getChatCode(msg.sender, _friendAddress);
